...
Options:
  ...
  --force-clean                      Remove .venv, the generated .calibrax.env, and repo-local test artifacts
  --dry-run                          Print the resolved backend and uv commands without changing files
  --help, -h                         Show this help

Notes:
  - Calibrax uses uv for all repo-maintained setup and test workflows.
  ...
  - The generated backend file is .calibrax.env. User-owned .env is not modified.
EOF
}
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    echo "uv ${SYNC_ARGS[*]}"
    echo "uv run --no-sync python -m substrax.runtime.managed_env write --prefix CALIBRAX --backend $BACKEND --output $MANAGED_ENV_FILE ${MANAGED_ENV_EXTRAS[*]}"
    exit 0
fi
...
uv "${SYNC_ARGS[@]}"
uv run --no-sync python -m substrax.runtime.managed_env write --prefix CALIBRAX --backend "$BACKEND" \
...
cat <<EOF
Calibrax environment synced.
Resolved backend: $BACKEND
Managed backend file: $MANAGED_ENV_FILE

Next steps:
  source ./activate.sh
  python -m substrax.runtime.managed_env show --prefix CALIBRAX --env-file .calibrax.env --user-env .env --user-env .env.local
EOF
